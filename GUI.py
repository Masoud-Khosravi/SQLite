...
def fill_list1(items, columns):
    lc = len(columns)
    list1["columns"] = list(range(lc))
    for item in columns:
        idx = str(columns.index(item))
        list1.column(idx, width=80, anchor=CENTER, stretch=True)
        list1.heading(idx, text=item)
    for item in items:
        # print(len(item))
        item_list = []
        for index in range(len(item)):
            item_list.append(item[index])
        list1.insert('', 'end', values=item_list)
    st.configure("list1", rowheight=100)


def view_sales_invoices():
    clear_list1()
    sales = db.view_sells()
    cols = ["ID", "Total Price", " User", "Date"]
    fill_list1(sales, cols)
...
def view_customers():
    ...
    # print(customers)
    fill_list1(customers, cols)


def view_sellers():
    clear_list1()
    sellers = db.view_sellers()
    cols = ["ID", "Name", "Family", "Sex", "Address", "Is_Seller"]
    fill_list1(sellers, cols)
...
st.configure("Treeview", background="#383838",
             foreground="white")
list1 = Treeview(SQLWindow, show='headings', height=10)

list1["columns"] = list(range(6))
f_items = []
for i in range(6):
    list1.column(i, width=80, anchor=CENTER, stretch=True)
    list1.heading(i, text=str(i))
    f_items.append(i)
list1.insert('', 'end', values=f_items)  # this item is for test and make width=constant
...
def new_window(is_buy=False):
    ...
    def save_fuc():
        ...
        if is_buy:
            # print("Buy")
            invoice_id = db.add_buy(user_index, ent_sum.get(), Now)
            if invoice_id > 0:
                for child in list_invoice.get_children():
                    ...
                    value = int(item[4])
                    price = float(item[5])
                    db.add_buy_details(invoice_id, ware_id, value, price)
        else:
            # print("Sell")
            invoice_id = db.add_sell(user_index, ent_sum.get(), Now)
            if invoice_id > 0:
                for child in list_invoice.get_children():
                    item = list_invoice.item(child)["values"]
                    ware_id = int(item[0])
                    value = int(item[4])
                    price = float(item[5])
                    db.add_sell_details(invoice_id, ware_id, value, price)
    ...
    def changed_wares(event):
        ...
        price = str(db.get_sell_price(ware_index))
    ...
    def changed_combos(event):
        ...
        if counter == 2:
            ...
            result = db.view_wares(brand_index, cat_index)
